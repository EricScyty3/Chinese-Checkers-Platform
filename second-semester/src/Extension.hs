...
finalSelection :: GameTree -> GameTreeStatus -> (Maybe Int, Maybe Pico) -> IO (Board, [Pos], HistoryTrace, [KillerMoves])
...
getResultsUnderControl :: GameTree -> GameTreeStatus -> (Maybe Int, Maybe Pico) -> IO (GameTree, [Double], HistoryTrace, [KillerMoves])
...
getResultsUnderControl tree status@(_, pi, _, _, _, _, _, _, _) (Nothing, Just seconds) =
                                                             do -- first get the current time for later check
                                                                ...
                                                                (ntree, _, nht, kms) <- timeLimits tree status (startTime, seconds)
...
timeLimits :: GameTree -> GameTreeStatus -> (UTCTime, Pico) -> IO (GameTree, BoardIndex, HistoryTrace, [KillerMoves])
timeLimits tree s@(_, pi, bi, board, ps, pn, ht, cons, (eval, depth, kms)) (start, duration) =
    do -- check if the time is exceeded
       ...
       if interval >= duration then return (tree, bi, ht, kms)
       -- otherwise, keep processing
       else let n@(newGen, newTree, newIdx, newHistory, nkms) = evalState (mcts tree) s
            in  n `seq` timeLimits newTree (newGen, pi, newIdx, board, ps, pn, newHistory, cons, (eval, depth, nkms)) (start, duration)
