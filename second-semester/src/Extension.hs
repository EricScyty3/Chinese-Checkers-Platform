...
finalSelection :: GameTree -> GameTreeStatus -> MCTSControl -> IO (Board, [Pos], HistoryTrace, [KillerMoves])
finalSelection tree s@(_, pi, _, eboard, iboards, pn, _, _, _) control =
                                                                   do -- pass the arguments to the decision function controlled by certain threshold 
                                                                      ...
                                                                      (ntree, nht, kms) <- getResultsUnderControl tree s control
                                                                      ...
                                                                      if null children then do printEoard eboard
                                                                              ...
                                                                              return $ newBoard `par` newInternalState `pseq` (newBoard, newInternalState, nht, kms)
-- compute the MCTS with certain threshold to control the progress
getResultsUnderControl :: GameTree -> GameTreeStatus -> MCTSControl -> IO (GameTree, HistoryTrace, [KillerMoves])
...
getResultsUnderControl tree status@(_, pi, _, _, _, _, _, _, _) (Nothing, Just seconds) =
                                                             do -- first get the current time for later check
                                                                ...
                                                                (ntree, nht, kms) <- timeLimits tree status (startTime, realToFrac seconds)
                                                                return (ntree, nht, kms)
...
timeLimits :: GameTree -> GameTreeStatus -> (UTCTime, Pico) -> IO (GameTree, HistoryTrace, [KillerMoves])
timeLimits tree s@(_, pi, bi, board, ps, pn, ht, cons, (eval, depth, kms)) (start, duration) =
    ...
    in  do -- check if the time is exceeded
           ...
           if interval >= duration then return (newTree, newHistory, nkms)
           -- otherwise, keep processing
           else timeLimits newTree (newGen, pi, newIdx, board, ps, pn, newHistory, cons, (eval, depth, nkms)) (start, duration)
